...
Adc_ConfigType* Adc_ThisConfiguration = ((void *)0);
...
void Adc_Init(Adc_ConfigType* Adc_Config)
{
 ...
 if (Adc_Config == ((void *)0))
    {
  Det_ReportError(0x123, 0x00,
        0x00,0x0E);
        ...
        for(int i=0 ; i<Adc_Config->Adc_NbChannels; i++)
         {
            ...
            (ADCON0 |= (Adc_Config->ArrayOfAdcChannels[i] << 3));
            (ADCON0 |= ((Adc_Config->Adc_Prescale & 0x03) << 6));
            (ADCON1 |= ((Adc_Config->Adc_Prescale & 0x04) << 4));
            (ADCON1 |= (Adc_Config->Adc_ResultAlignment << 7));
         }
 ...
 if (DataBufferPtr == ((void *)0))
  ...
  Det_ReportError(0x123, 0x00,
...

    {
        ...
        {
         ADCON0 &= 0x11000101;
         (ADCON0 |= (Adc_ThisConfiguration->ArrayOfAdcChannels[0] << 3));
         if(Adc_ThisConfiguration->Adc_GroupAccessMode == ADC_ACCESS_MODE_STREAMING)
         {
            for(uint8_t i = 0; i<Adc_ThisConfiguration->Adc_NbSamples; i++)
            {
               ...
               *DataBufferPtr++ = ((ADRESH << 8) + ADRESL)/2;
            }
