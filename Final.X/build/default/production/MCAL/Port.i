...
void Port_Init(Port_ConfigType* ConfigPtr)
{
 ...
 if (ConfigPtr == ((void *)0))
    {
  Det_ReportError(0x124, 0x00, 0x00,0x10);
 } else

    {
        volatile Port_PinType* Port_Dir_Ptr,*Port_Ptr = ((void *)0);
        Port_Dir_Ptr = &(*((volatile uint8*)(((uint8)0x085)+((uint8)ConfigPtr->Port))));
        Port_Ptr = &(*((volatile uint8*)(((uint8)0x005)+((uint8)ConfigPtr->Port))));


        if (ConfigPtr->PinDirection == PORT_PIN_OUT)
        {
            (*Port_Dir_Ptr&=(~(1<<ConfigPtr->Pin)));
# 60 "MCAL/Port.c"
            (*Port_Ptr&=(~(1<<ConfigPtr->Pin)));
