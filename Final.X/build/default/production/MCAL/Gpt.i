...
# 1 "MCAL/../common_macros.h" 1
# 31 "MCAL/Port.h" 2



# 1 "MCAL/../micro_config.h" 1
...
# 1 "MCAL/interrupt.h" 1
# 22 "MCAL/interrupt.h"
# 1 "MCAL/Port.h" 1
# 22 "MCAL/interrupt.h" 2
# 33 "MCAL/interrupt.h"
typedef enum {
    TIMER,
    ADC,
...
Gpt_ConfigType* Gpt_ThisConfiguration_1 = ((void *)0) ;
Gpt_ConfigType* Gpt_ThisConfiguration_2 = ((void *)0) ;
  ...
  Det_ReportError(0x100, 0x00,
...

    {
      ...
      if(ConfigPtr->TickRate>39)
      {
          ...
          Gpt_ReloadValue=(65535-(10000000/(ConfigPtr->TickRate*4*1)));
      }
      else if(ConfigPtr->TickRate>20)
      {
          T1CON = (GPT_PRE_2 <<4);

          Gpt_ReloadValue=(65535-(10000000/(ConfigPtr->TickRate*4*2)));
      }
      else if(ConfigPtr->TickRate>10)
      {
          T1CON = (GPT_PRE_4 <<4);

          Gpt_ReloadValue=(65535-(10000000/(ConfigPtr->TickRate*4*4)));
      }
      else if(ConfigPtr->TickRate>5)
      {
          T1CON = (GPT_PRE_8 <<4);

          Gpt_ReloadValue=(65535-(10000000/(ConfigPtr->TickRate*4*8)));
      }
    ...
    else if(ConfigPtr->Module == TIMER2 && ConfigPtr->Mode == PWM_MODE)
    {
        if(ConfigPtr->TickRate>10000)
      {
          ...
          PR2 = 10000000/(ConfigPtr->TickRate*4*1)-1;
      }
      else if(ConfigPtr->TickRate>2500)
      {
          T2CON = ((GPT_PRE_4/2) <<0);

          PR2 = 10000000/(ConfigPtr->TickRate*4*4)-1;
      }
      else if(ConfigPtr->TickRate>600)
      {
          T2CON = ((GPT_PRE_16/2) <<0);

          PR2 = 10000000/(ConfigPtr->TickRate*4*16)-1;
      }
