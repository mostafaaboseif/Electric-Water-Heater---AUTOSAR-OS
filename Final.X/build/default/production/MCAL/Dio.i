...
void Dio_WriteChannel(Dio_ChannelType ChannelId, Dio_LevelType Level) {
 ...
 if (9 <= ChannelId)
 {
  ...
  Det_ReportError(0x120, 0x00,
    0x01, 0x0A);
 }
    else

    {
        volatile Dio_PortType * Port_Ptr = ((void *)0);
        Port_Ptr = &(*((volatile uint8*)(((uint8)0x005)+((uint8)Dio_ConfigGroupChannel.Channels[ChannelId].Port))));

        if (Level == HIGH)
            (*Port_Ptr|=(1<<Dio_ConfigGroupChannel.Channels[ChannelId].Channel));
        else if (Level == LOW)
            (*Port_Ptr&=(~(1<<Dio_ConfigGroupChannel.Channels[ChannelId].Channel)));
    }
}
...
Dio_LevelType Dio_ReadChannel(Dio_ChannelType ChannelId)
{
 ...
 if (9 <= ChannelId)
 {
  ...
  Det_ReportError(0x120, 0x00,
    ...
    {
        volatile Dio_PortType * Pin_Ptr = ((void *)0);
        Dio_LevelType output = LOW;
        Pin_Ptr = &(*((volatile uint8*)(((uint8)0x005)+((uint8)Dio_ConfigGroupChannel.Channels[ChannelId].Port))));
...
void Dio_WritePort ( Dio_PortType PortId, Dio_PortLevelType Level )
{
    volatile uint8_t * Port_Ptr = ((void *)0);
    Port_Ptr = &(*((volatile uint8*)(((uint8)0x005)+((uint8)PortId))));
        (*Port_Ptr=Level);
}
