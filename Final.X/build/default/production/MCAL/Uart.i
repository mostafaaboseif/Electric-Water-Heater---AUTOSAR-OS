...
Uart_ConfigType* Uart_ThisConfiguration = ((void *)0);
uint8_t* RX_Data;
...
# 70 "MCAL/Uart.c"
    {
        ...
        Port_Init(&Uart_PinsConfig[1]);
         ...
         uint32_t Uart_ReloadValue = (10000000 - ConfigPtr->Baudrate*64)/(ConfigPtr->Baudrate*64);
        if(Uart_ReloadValue>255)
        {
          Uart_ReloadValue = (10000000 - ConfigPtr->Baudrate*16)/(ConfigPtr->Baudrate*16);
          (TXSTA|=(1<<2));
        }
        if (Uart_ReloadValue<=255)
          SPBRG = Uart_ReloadValue;
